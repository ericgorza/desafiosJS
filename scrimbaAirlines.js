...
function sortByDuration(data, flightLength){
    let sortedDataAsc = data.sort((a,b) => a.duration - b.duration).map((item) => {return `${item.title},${item.duration}`})
    let sortedDataDesc = data.sort((a,b) => b.duration - a.duration).map((item) => {return `${item.title},${item.duration}`})
    if(flightLength <= 60){

...
function createDescriptionsFor(data){
    const descr = data.map((item) => {
        let podcastTitle = item.title
        let podcastDuration = item.duration
        let podcastTags = item.tags.join(",")
        let podcastHosts = item.hosts.join(",")
        return `${podcastTitle} is a ${podcastDuration} minute ${podcastTags} podcast hosted by ${podcastHosts}.`
    })
    for(let i = 0; i < data.length; i++){
        data[i]['description'] = descr[i]
    }
    return data;
}

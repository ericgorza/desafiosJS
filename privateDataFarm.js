...
function transformData(data){
    let newDict = []
    data.forEach(item => {
        let nameToAdd = item["name"]["title"] + " " + item["name"]["first"] + " " + item["name"]["last"]
        let dateToAdd = new Date(item["dob"]["date"]).toDateString()
        newDict.push({"name":nameToAdd,"date":dateToAdd})
    });
    return newDict;
}

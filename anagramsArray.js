...
const anagrams = [
    ...
    "Bob Ziroll Scrimba reacheT"
]

function isAnagramInArray(anagram, arr){
    let anagramList = []
    let sortedAnagram = anagram.split("").filter(item => item != " ").sort().join("").toLowerCase()
    arr.forEach(item => {
        let sortedString = item.split("").filter(item => item != " ").sort().join("").toLowerCase()
        if(sortedString.length == sortedAnagram.length){
            if(sortedString == sortedAnagram){
